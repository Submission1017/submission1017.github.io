  ...
  this.analyze = function (context) {
    ...
    for (var i = 0; i < this.nodes.length; ++i) {
      ...
      var origin = new Point(
        this.origin.x + space * (i + 1.5) + width,
        this.origin.y
      );
      node.origin = origin;
      node.width = width;
    ...
    for (var i = 0; i < this.literature.length; ++i) {
      ...
      for (var j = 0; j < this.nodes.length; ++j) {
        for (var k = 0; k < this.nodes[j].nodelings.length; ++k) {
          if (
            paper.data[this.nodes[j].title] === this.nodes[j].nodelings[k].name
          ) {
            if (j === 0) {
              var awidth = this.nodes[j].nodelings[k].unit_height;
              var point1 = new Point(
                paper.origin.x + paper.width,
                paper.origin.y
              );
              var point2 = new Point(
                this.nodes[j].nodelings[k].origin.x,
                this.nodes[j].nodelings[k].origin.y +
                  awidth * this.nodes[j].nodelings[k].amount_of_connections
              );
              ...
              var awidth = this.nodes[j].nodelings[k].unit_height;
              var point1 = new Point(
                paper.connections[j - 1].point2.x + this.nodes[j - 1].width,
      ...
      for (var j = 0; j < this.nodes.length; ++j) {
        for (var k = 0; k < this.nodes[j].nodelings.length; ++k) {
          ...
          if (
            ...
            if (j === 0) {
              var awidth = this.nodes[j].nodelings[k].unit_height;
              paper.connections[j].point2 = new Point(
                this.nodes[j].nodelings[k].origin.x,
                this.nodes[j].nodelings[k].origin.y +
                  awidth * this.nodes[j].nodelings[k].amount_of_connections
              );
              ...
              var awidth = this.nodes[j].nodelings[k].unit_height;
              paper.connections[j - 1].point1 = new Point(
                paper.connections[j - 1].point2.x + this.nodes[j - 1].width,
                paper.connections[j - 1].point2.y
              );
              paper.connections[j].point2 = new Point(
    ...
    for (var i = 0; i < this.literature.length; ++i) {
      ...
      if (node1.origin.x > node2.origin.x) {
        if (this.nodes.indexOf(node2) - 1 >= 0)
          paper.connections[this.nodes.indexOf(node2) - 1].point2.x =
            node1.origin.x + node1.width;
        paper.connections[this.nodes.indexOf(node2)].point1.x = node1.origin.x;
        paper.connections[this.nodes.indexOf(node2)].point2.x =
          node2.origin.x + node2.width;
        if (this.nodes.indexOf(node2) + 1 < paper.connections.length)
          paper.connections[this.nodes.indexOf(node2) + 1].point1.x =
            node2.origin.x;
      } else if (node1.origin.x < node2.origin.x) {
        if (this.nodes.indexOf(node1) - 1 >= 0)
          paper.connections[this.nodes.indexOf(node1) - 1].point2.x =
            node2.origin.x + node2.width;
        paper.connections[this.nodes.indexOf(node1)].point1.x = node2.origin.x;
        paper.connections[this.nodes.indexOf(node1)].point2.x =
          node1.origin.x + node1.width;
        if (this.nodes.indexOf(node1) + 1 < paper.connections.length)
          paper.connections[this.nodes.indexOf(node1) + 1].point1.x =
            node1.origin.x;
      }
    }
  };
...
function Node(title, data) {
  ...
  this.resizeNodelings = function () {
    ...
    var unit_height =
      this.height / (total_amount + (this.nodelings.length - 1));

    for (var i = 0; i < this.nodelings.length; ++i) {
      this.nodelings[i].height =
        unit_height * (this.nodelings[i].amount + 1) - unit_height / 2;
  ...
  this.draw = function (context, set) {
    ...
    for (var i = 0; i < this.nodelings.length; ++i) {
      ...
      if (i === 0) {
        ...
        nodeling.origin = new Point(
          this.origin.x,
          this.origin.y + total_height + nodeling.unit_height / 2
        );
        total_height += nodeling.height + nodeling.unit_height / 2;
      }

      if (!set) nodeling.draw(context);
    }
  };
  ...
  this.move = function (newOrigin) {
    ...
    if (
      parseInt(this.origin.x) === parseInt(newOrigin.x) ||
      parseInt(this.origin.x) + 1 === parseInt(newOrigin.x) ||
      parseInt(this.origin.x) === parseInt(newOrigin.x) + 1
    ) {
      return true;
    } else {
      return false;
