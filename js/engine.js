...
function showLoadingScreen(msg) {
    ...
    context.fillText(
        msg,
        vis.origin.x + vis.width / 2,
        vis.origin.y + vis.height / 2
    );
    context.restore();
}
        ...
        if (vis.literature[i].isHit(new Point(x, y))) {
            ...
            for (var j = 0; j < vis.nodes.length; ++j) {
                for (var k = 0; k < vis.nodes[j].nodelings.length; ++k) {
                    if (
                        vis.literature[i].data[vis.nodes[j].title] ===
                        vis.nodes[j].nodelings[k].name
                    ) {
                        if (
                            useCustomHighlights &&
                            vis.nodes[j].nodelings[k].name in highlights
                        )
                            vis.nodes[j].nodelings[k].background =
                                highlights[vis.nodes[j].nodelings[k].name];
                        else vis.nodes[j].nodelings[k].background = highlight_color;
                        somethingIsHit = true;
                    } else {
                        vis.nodes[j].nodelings[k].background =
                            vis.nodes[j].nodelings[k].default_background;
                    }
                }
            }
        } else {
        ...
        for (var i = 0; i < vis.nodes.length; ++i) {
            for (var j = 0; j < vis.nodes[i].nodelings.length; ++j) {
                if (vis.nodes[i].nodelings[j].isHit(new Point(x, y))) {
                    tooltipIsVisible = true;
                    label =
                        vis.nodes[i].nodelings[j].name +
                        ' (' +
                        vis.nodes[i].nodelings[j].amount +
                        ')';
                    $('#vis').css({cursor: 'pointer'});

                    if (
                        useCustomHighlights &&
                        vis.nodes[i].nodelings[j].name in highlights
                    )
                        vis.nodes[i].nodelings[j].background =
                            highlights[vis.nodes[i].nodelings[j].name];
                    else vis.nodes[i].nodelings[j].background = highlight_color;

                    for (var k = 0; k < vis.literature.length; ++k) {
                        if (
                            vis.literature[k].data[vis.nodes[i].title] ===
                            vis.nodes[i].nodelings[j].name
                        ) {
                            if (useCustomHighlights)
                            ...
                            vis.literature[k].background =
                                vis.literature[k].default_background;
                        }
                    }
                } else {
                    vis.nodes[i].nodelings[j].background =
                        vis.nodes[i].nodelings[j].default_background;
                }
            }
        }
    }
        ...
        for (var j = 0; j < vis.nodes.length; ++j) {
            ...
            for (var k = 0; k < vis.nodes[j].nodelings.length; ++k) {
                if (focused_nodes.indexOf(vis.nodes[j].nodelings[k]) != -1) {
                    if (
                        paper.data[vis.nodes[j].nodelings[k].parent] ===
                        vis.nodes[j].nodelings[k].name
                    ) {
                        flag = true;
                        break;
...
function moveIndex(num, index) {
    ...
    vis.switchNodes(
        vis.nodes[index],
        vis.nodes[index + num],
        new Point(vis.nodes[index + num].origin.x, vis.nodes[index + num].origin.y),
        new Point(vis.nodes[index].origin.x, vis.nodes[index].origin.y)
    );
}
